...
import { Star, Moon, Sun, BookOpen, CheckCircle, Award, ChevronLeft, ChevronRight, PenTool, User, CloudUpload, ArrowRight, Book, Settings, Camera, X, Heart, MessageCircle, List, Calendar, Palette, Sparkles, Loader2, Trophy, AlertTriangle } from 'lucide-react';

// --- DATA: KOLEKSI DOA (SUMBER: HISNUL MUSLIM INDONESIA) ---
const DAFTAR_DOA = [
  { 
  ...
  { 
    ...
    arti: "Ya Allah, bagi-Mu segala puji, Engkau-lah yang memakaikan pakaian ini kepadaku. Aku memohon kepada-Mu kebaikannya dan kebaikan apa saja yang dibuat untuknya, dan aku berlindung kepada-Mu dari keburukannya dan keburukan apa saja yang dibuat untuknya." 
  },
  { 
  ...
  { 
    ...
    arab: "اللَّهُمَّ اجْعَلْ فِي قَلْبِي نُورًا، وَفِي لِسَانِي نُورًا", 
    latin: "Allahummaj'al fii qalbii nuuraa, wa fii lisaanii nuuraa.", 
    arti: "Ya Allah, jadikanlah cahaya di hatiku dan cahaya di lisanku." 
  },
  { 
  ...
  { 
    ...
    arti: "Ya Allah, Tuhan Pemilik panggilan yang sempurna (adzan) ini dan shalat yang ditegakkan. Berilah Al-Wasilah (derajat di surga) dan fadhilah kepada Nabi Muhammad. Dan bangkitkanlah beliau di tempat yang terpuji yang telah Engkau janjikan." 
  },
  { 
    judul: "15. Doa Istiftah", 
  ...
  { 
    judul: "17. Doa Bangkit Dari Ruku'", 
    arab: "سَمِعَ اللهُ لِمَنْ حَمِدَهُ ... رَبَّنَا وَلَكَ الْحَمْدُ", 
    latin: "Sami'allaahu liman hamidah ... Rabbanaa wa lakal hamdu.", 
    arti: "Allah Maha Mendengar orang yang memuji-Nya. Ya Tuhan kami, bagi-Mu segala puji." 
  },
  { 
    judul: "18. Doa Sujud", 
  ...
  { 
    judul: "19. Doa Duduk Di Antara Dua Sujud", 
  ...
  { 
    judul: "20. Bacaan Tasyahhud", 
    arab: "التَّحِيَّاتُ لِلَّهِ وَالصَّلَوَاتُ وَالطَّيِّبَاتُ", 
    latin: "At-tahiyyaatu lillaahi wash-shalawaatu wath-thayyibaat.", 
    arti: "Segala penghormatan, shalawat, dan kebaikan hanya milik Allah." 
  },
  { 
    judul: "21. Salawat Nabi", 
    arab: "اللَّهُمَّ صَلِّ عَلَى مُحَمَّدٍ وَعَلَى آلِ مُحَمَّدٍ", 
    latin: "Allahumma shalli 'alaa Muhammad wa 'alaa aali Muhammad.", 
    arti: "Ya Allah, berilah shalawat kepada Muhammad dan keluarga Muhammad." 
  },
  { 
    judul: "22. Doa Sebelum Salam", 
    arab: "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنْ عَذَابِ الْقَبْرِ، وَمِنْ عَذَابِ جَهَنَّمَ", 
    latin: "Allahumma inni a'udzu bika min 'adzabil qabri, wa min 'adzabi jahannama.", 
    arti: "Ya Allah, aku berlindung kepada-Mu dari siksa kubur dan dari siksa neraka Jahanam." 
  },
  { 
    judul: "23. Dzikir Setelah Salam", 
    arab: "أَسْتَغْفِرُ اللهَ (3x) اللَّهُمَّ أَنْتَ السَّلَامُ وَمِنْكَ السَّلَامُ", 
    latin: "Astaghfirullah (3x), Allahumma antas-salaam wa minkas-salaam.", 
    arti: "Aku memohon ampun kepada Allah (3x). Ya Allah, Engkau Mahasejahtera, dan dari-Mu kesejahteraan." 
  },
  { 
  ...
  { 
    judul: "29. Doa Tamu Berpuasa", 
    arab: "أَفْطَرَ عِنْدَكُمُ الصَّائِمُونَ، وَأَكَلَ طَعَامَكُمُ الْأَبْرَارُ", 
    latin: "Afthara 'indakumush-shaa-imuun, wa akala tha'aamakumul abraar.", 
    arti: "Semoga orang-orang yang berpuasa berbuka di tempat kalian dan orang-orang yang baik memakan makanan kalian." 
  },
  { 
    judul: "30. Doa Jika Dicela Saat Puasa", 
  ...
  { 
    judul: "31. Doa Melihat Buah Awal Musim", 
    arab: "اللَّهُمَّ بَارِكْ لَنَا فِي ثَمَرِنَا", 
    latin: "Allahumma baarik lanaa fii tsamarinaa.", 
    arti: "Ya Allah, berkahilah kami pada buah-buahan kami." 
  },
  { 
  ...
  { 
    ...
    latin: "Subhaanal-ladzi sakhkhara lanaa hadzaa wa maa kunnaa lahu muqriniin.", 
  ...
  { 
    ...
    arab: "لَا إِلَهَ إِلَّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ", 
    latin: "Laa ilaaha illallaahu wahdahu laa syariika lah, lahul mulku wa lahul hamdu.", 
    arti: "Tidak ada Tuhan yang berhak disembah selain Allah semata, tiada sekutu bagi-Nya. Bagi-Nya kerajaan dan segala puji." 
  }
];
...
export default function App() {
  ...
  // --- VALIDASI URL ---
  const isScriptUrlValid = (url) => {
    if (!url) return false;
    return url.includes('script.google.com') && url.endsWith('/exec');
  };
  ...
  const sendToGoogleSheet = async (e) => {
    ...
    if (!scriptUrl) {
      alert("⚠️ Link Laporan Guru belum diisi!\nMasuk ke menu Pengaturan (ikon Gear) di halaman depan.");
      return;
    }

    if (!scriptUrl.includes('/exec')) {
      alert("⚠️ Link Script SALAH!\nLink harus berakhiran '/exec'.\n\nAnda mungkin memasukkan link Editor (/edit). Mohon Deploy ulang dan pilih 'Web App'.");
      return;
    }
    ...
    try {
      ...
      alert("❌ Gagal mengirim data.\n\nKemungkinan penyebab:\n1. Tidak ada internet.\n2. Link Script Guru salah (Pastikan 'Anyone' bisa akses).");
    } finally {
          ...
          <div className="absolute inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl overflow-y-auto max-h-[90vh]">
              ...
              <div className="space-y-4">
                ...
                {/* VALIDASI INPUT LINK SCRIPT */}
                <div>
                  <label className="text-xs font-bold text-slate-500 mb-1 block">Link Script Guru (Harus berakhiran /exec)</label>
                  <input 
                    type="text" 
                    className={`w-full p-2 border rounded-lg text-xs ${studentProfile.scriptUrl && !isScriptUrlValid(studentProfile.scriptUrl) ? 'border-red-500 bg-red-50 text-red-600' : ''}`} 
                    placeholder="https://script.google.com/.../exec" 
                    value={studentProfile.scriptUrl} 
                    onChange={(e) => setStudentProfile(prev => ({...prev, scriptUrl: e.target.value}))} 
                  />
                  {studentProfile.scriptUrl && !isScriptUrlValid(studentProfile.scriptUrl) && (
                    <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={10} /> Link sepertinya salah. Harus berakhiran '/exec'.</p>
                  )}
                </div>
  ...
  return (
    <div className={`min-h-screen ${currentTheme.bg} font-sans text-slate-800 pb-20`}>
      {/* HEADER: Relative agar ikut scroll */}
        ...
        {view === 'journal' && (
          <div className="space-y-4 pb-10">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
             ...
             <div className="bg-gradient-to-br from-[#1e293b] to-[#0f172a] p-4 rounded-3xl shadow-lg border border-indigo-900 relative overflow-hidden text-white">
                ...
                <div className="flex justify-between items-center relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-sm"><Moon size={24} className="text-yellow-200 fill-yellow-200" /></div>
                    <div>
                      <h3 className="font-bold text-white flex items-center gap-2">Salat Malam</h3>
                      <p className="text-xs text-indigo-200">Bangun Malam (Tahajud)</p>
                    </div>
                  </div>
                  <div onClick={() => !currentData.validated && toggleCheck(activeDay, 'salatMalam')} className={`w-10 h-10 rounded-full border-2 flex items-center justify-center cursor-pointer transition-all ${currentData.salatMalam ? 'bg-yellow-400 border-yellow-400 text-yellow-900 scale-110' : 'border-indigo-400/50 bg-white/5'}`}>
            ...
            <div className="grid grid-cols-3 gap-2">
               {[{ id: 'tarawih', label: 'Tarawih' }, { id: 'witir', label: 'Witir' }, { id: 'tadarus', label: 'Tadarus' }].map(ibadah => (
                  <div key={ibadah.id} onClick={() => !currentData.validated && toggleCheck(activeDay, ibadah.id)} className={`flex flex-col items-center justify-center p-3 rounded-2xl border-2 cursor-pointer text-center ${currentData[ibadah.id] ? `bg-${currentTheme.accent}-50 border-${currentTheme.accent}-500` : 'bg-white border-slate-100'}`}>
                    <span className={`font-bold text-xs ${currentData[ibadah.id] ? currentTheme.text : 'text-slate-500'}`}>{ibadah.label}</span>
                  </div>
               ))}
            </div>
            ...
            <div className={`bg-gradient-to-br from-white to-${currentTheme.accent}-50 p-5 rounded-3xl shadow-sm border border-${currentTheme.accent}-100 space-y-3`}>
               ...
               {[{key: 'kosakata', label: 'Hafal Kosakata (+5)'}, {key: 'bantuIbu', label: 'Bantu Ibu (+5)'}].map(item => (
                 <div key={item.key} onClick={() => !currentData.validated && toggleCheck(activeDay, item.key)} className={`p-3 rounded-xl border cursor-pointer flex justify-between items-center ${currentData[item.key] ? `${currentTheme.bg} border-${currentTheme.accent}-300` : 'bg-white border-slate-100'}`}>
                    ...
                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${currentData[item.key] ? `${currentTheme.header} border-transparent` : 'border-slate-300'}`}>{currentData[item.key] && <CheckCircle size={12} className="text-white" />}</div>
                 </div>
               ))}

               {/* Dropdown Hafalan Doa */}
               <div className="p-3 bg-white rounded-2xl border border-slate-100">
                 <div className="flex items-center gap-2 mb-2"><List size={18} className={`text-${currentTheme.accent}-500`} /><span className="text-xs font-bold text-slate-600">Menghafal Doa (+10)</span></div>
                 <select className={`w-full text-xs p-2 bg-slate-50 rounded-lg border-none outline-none text-slate-700`} value={currentData.hafalDoa || ""} onChange={(e) => updateField(activeDay, 'hafalDoa', e.target.value)} disabled={currentData.validated}>
                    <option value="">-- Pilih Doa --</option>
                    {DAFTAR_DOA.map((d, i) => <option key={i} value={d.judul}>{d.judul}</option>)}
                 </select>
               </div>
               ...
               <div className="p-3 bg-white rounded-2xl border border-slate-100">
                  <div className="flex justify-between mb-2">
                    <span className="text-xs font-bold text-slate-600">Amalan Lain (+5)</span>
                    <label className="flex items-center gap-1"><input type="checkbox" checked={currentData.amalanLainCheck || false} onChange={() => toggleCheck(activeDay, 'amalanLainCheck')} disabled={currentData.validated} /><span className="text-[10px]">Selesai</span></label>
                  </div>
                  <input type="text" className="w-full text-xs p-2 bg-slate-50 rounded-lg border border-slate-200 outline-none" placeholder="Contoh: Sedekah..." value={currentData.amalanLain || ""} onChange={(e) => updateField(activeDay, 'amalanLain', e.target.value)} disabled={currentData.validated} />
               </div>
            </div>
                ...
                <div className="mb-4 text-left">
                  ...
                  {!selectedImage ? (
                    ...
                    <div className="relative w-full h-40 rounded-xl overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center">
                      {isCompressing ? (
                        <div className="text-center"><Loader2 size={24} className="animate-spin text-blue-500 mx-auto" /><span className="text-[10px] text-slate-500">Mengecilkan foto...</span></div>
                      ) : (
                        <><img src={base64Image} alt="Preview" className="w-full h-full object-cover" /><button onClick={removeImage} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full"><X size={14} /></button></>
                      )}
                    </div>
                  )}
                </div>
